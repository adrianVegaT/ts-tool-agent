...
export async function runAgent(question: string): Promise<AgentResponse> {
    ...
    if (response.stop_reason === 'tool_use') {
        ...
        const newTokensUsed = tokensUsed + finalResponse.usage.input_tokens + finalResponse.usage.output_tokens


        const finalBlock = finalResponse.content[0]
        const text = finalBlock?.type === 'text' ? finalBlock.text : ''
    ...
    const newTokensUsed = tokensUsed + response.usage.input_tokens + response.usage.output_tokens
